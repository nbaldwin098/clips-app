...
assert(cadenceSrc.includes('mixVideoFeedRows'), 'home video grid gets in-feed ads')
assert(adEng.includes("placement === 'video-feed'"), 'video feed is a real placement')
assert(adEng.includes('watchBannerAllowed'), 'watch page can show a banner under the player')
const shelvesSrc2 = readFileSync(new URL('../src/components/MediaShelves.jsx', import.meta.url), 'utf8')
assert(shelvesSrc2.includes("'video-feed'"), 'video shelf inserts ads between cards')
const watchSrc3 = readFileSync(new URL('../src/components/WatchPage.jsx', import.meta.url), 'utf8')
assert(watchSrc3.includes('watchBannerAllowed()'), 'watch page renders the display banner')
assert(cadenceSrc.includes("provider: 'exoclick'"), 'clip and pic feeds use the ExoClick display zone')
assert(cadenceSrc.includes('EXOCLICK_BANNER_ZONE = \'6010930\''), 'video strips use the ExoClick banner zone')
...
assert(adUnitsSrc.includes("variant === 'pic-row'"), 'pic full ads use a mosaic row layout')
assert(adUnitsSrc.includes('aspect-[3/1]'), 'pic full ads match photo row height')
assert(adUnitsSrc.includes('format="banner"'), 'video and pic mosaic ads use the banner unit')
assert(adUnitsSrc.includes('format="display"'), 'clip cards use the display unit at native size')
assert(adUnitsSrc.includes('placementAdsAllowed'), 'pic banners fall back to ExoClick when no campaign is live')
assert(shelvesSrc2.includes('col-span-full'), 'video ads span the grid as a banner strip')
...
assert(picsPageSrc.includes('format="display"'), 'pic viewer ads use the display unit contained in the square')
...
assert(watchSrc3.includes('format="banner"'), 'watch banner uses the banner format')
