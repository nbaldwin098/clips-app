...
import { parseVastXml, parseVastClock, videoInStreamBreaks, youtubeMidrollBreaks, EXOCLICK_VAST_URL, EXOCLICK_LIVE_CREATOR_VAST_URL, YT_MIDROLL_MIN_SEC, VIDEO_FIRST_AD_SEC } from '../src/lib/vastAds.js'
...
assert(youtubeMidrollBreaks(YT_MIDROLL_MIN_SEC - 1).length === 0, 'no 8-minute break under 8 minutes')
...
assert(adEng.includes("placement === 'video-feed'") && adEng.includes('return false'), 'video-feed display boxes are forced off')
assert(adEng.includes('watchBannerAllowed') && /watchBannerAllowed\(\) \{\s*return false/.test(adEng), 'watch page never shows a banner under the player')
...
assert(adUnitsSrc.includes('format="display"'), 'clip and pic cards use the display unit at native size')
...
assert(exoSrc.includes("format === 'banner'"), 'banner and display units stay on their own IAB sizes')
assert(shortsFeedSrc.includes('format="display"'), 'clip player ads stay at native display size')
assert(shortsFeedSrc.includes('format="banner"'), 'clip banners use the banner format')
