...
assert(dashStudioSrc.includes('bg-white'), 'creator studio uses white background')
...
assert(appSrc.includes("isCreatorStudio ? 'bg-white'"), 'app paints white shell behind creator studio')
...
const audienceMapLib = readFileSync(new URL('../src/lib/bubbleMapData.js', import.meta.url), 'utf8')
assert(audienceMapLib.includes('buildAudienceBubbleMap'), 'audience bubble map builds anonymous user graph')
assert(audienceMapLib.includes('anonymizeUserId'), 'audience bubble map anonymizes viewers')
const bubbleMapUi = readFileSync(new URL('../src/components/studio/InteractionBubbleMap.jsx', import.meta.url), 'utf8')
assert(bubbleMapUi.includes('Holders Bubble Map'), 'bubble map UI is holders-style audience map')
assert(bubbleMapUi.includes('layoutBubbleRing'), 'bubble map lays out users on a ring')
assert(bubbleMapUi.includes('stroke="#ffffff"'), 'interaction links are white lines')
