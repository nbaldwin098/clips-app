...
import { mixClipFeedRows, mixPicFeedRows } from '../src/lib/feedAdCadence.js'
...
assert(!vastHookSrc.includes('skipPreroll'), 'preroll always runs at video start')
...
assert(!parseVastXml('<VAST version="3.0"></VAST>'), 'empty vast is not a fake ad')
{
  ...
  assert(mixed.some((r) => r.kind === 'ad'), 'clip reel inserts full-size ad slides')
  assert(mixed.length > fake.length, 'clip reel keeps ad rows separate from clips')
  assert(mixed.filter((r) => r.kind === 'item').length === fake.length, 'clip reel keeps every clip row')
  assert(mixed.some((r) => r.kind === 'item' && r.banner), 'clip rows can still carry banner flags')
}
{
  const fake = Array.from({ length: 50 }, (_, i) => ({ id: `p${i}` }))
  const mixed = mixPicFeedRows(fake)
  assert(mixed.some((r) => r.kind === 'ad'), 'pic reel inserts full-size ad slides')
  assert(mixed.length > fake.length, 'pic reel keeps ad rows separate from photos')
  assert(mixed.filter((r) => r.kind === 'item').length === fake.length, 'pic reel keeps every photo row')
}
...
assert(uploadModalSrc.includes('ErrorReportPrompt'), 'upload modal offers error report')
...
assert(storageSrc.includes('User uploads are never dropped'), 'saveImport never drops user uploads')
...
assert(healthSrcUpload.includes('Never delete a user upload'), 'hideBrokenMedia never deletes uploads')
assert(healthSrcUpload.includes('isFeedable(row)') || healthSrcUpload.includes('isUserUploadRecord(row)) return isFeedable'), 'purgeDeadCatalog drops unfeedable uploads')
assert(healthSrcUpload.includes('localStored === true'), 'empty-media uploads are not treated as playable')
assert(!healthSrcUpload.includes('Opaque public ids with empty media'), 'ghost empty uploads are not feedable')
assert(healthSrcUpload.includes('isUserUploadRecord(item)'), 'shared feeds require public URLs for user uploads')
assert(healthSrcUpload.includes('only belong in shared feeds when they have a public http URL')
  || healthSrcUpload.includes('Device-only / empty-media rows'), 'device-only uploads stay off the shared reel')
...
assert(uploadSrcEarly.includes('published.hosted'), 'share link only shown when hosted')
assert(uploadSrcEarly.includes("'Uploaded'") || uploadSrcEarly.includes('"Uploaded"'), 'upload success says Uploaded')
...
assert(shortsFeedSrcScroll.includes('key={`clips-player-${tab}-${focusId}`}'), 'clip player remounts per opened clip, not when ads skip')
assert(shortsFeedSrcScroll.includes('touch-pan-y'), 'clip tap layer allows vertical swipe')
assert(shortsFeedSrcScroll.includes('isFeedable'), 'clip reel skips ghost uploads before mixing ads')
...
assert(stageSrcScroll.includes('Do not include `n`'), 'skipping an ad does not rewind the reel to the first clip')
...
assert(shortsFeedSrc.includes('mixFeedAds'), 'clip player mixes scroll ads while scrolling')
assert(shortsFeedSrc.includes('ClipFeedAdSlide'), 'clip reel has full-size ad slides')
assert(shortsFeedSrc.includes('skipAdSlide'), 'clip reel drops empty ad slides without stalling')
assert(shortsFeedSrc.includes("kind === 'ad'"), 'clip reel renders ad slides')
assert(shortsFeedSrc.includes('InFeedAd'), 'clip ads fill the same ShortsCard as clips')
assert(shortsFeedSrc.includes('fillMobile'), 'clip ad slides match clip viewport size')
...
assert(adEng.includes('viewerWantsAds'), 'viewer ad opt-out is respected')
assert(/export function adsAreRunning\(viewerId/.test(adEng), 'ads respect viewer preference')
...
assert(adEng.includes('mixPicFeedRows'), 'pic feed mixes full-size ad slides')
...
assert(cadenceSrc.includes('CLIP_AD_GAPS'), 'clip reel inserts scroll ad gaps')
assert(cadenceSrc.includes('PIC_AD_GAPS'), 'pic reel inserts scroll ad gaps')
assert(shortsFeedSrc.includes('mixFeedAds'), 'clip player uses scroll ad cadence')
...
assert(adUnitsSrc.includes('InFeedAd'), 'in-feed ad component fills the scroll card')
assert(adUnitsSrc.includes('fillFrame'), 'scroll ads can fill the clip/pic frame')
assert(adUnitsSrc.includes('VideoPreroll'), 'watch preroll component remains')
assert(adUnitsSrc.includes('getAdViewerId'), 'placement banners respect viewer ad preference')
...
assert(picsPageSrc.includes('mixFeedAds'), 'pic viewer mixes scroll ads')
assert(picsPageSrc.includes('PicFeedAdSlide'), 'pic reel has full-size ad slides')
assert(picsPageSrc.includes('InFeedAd'), 'pic ads fill the same ShortsCard as pics')
assert(picsPageSrc.includes('Sponsored · swipe for next'), 'pic reel labels sponsored slides')
assert(!picsPageSrc.includes('pic-banner'), 'pic viewer does not put a small banner on the photo')
...
assert(!shortsFeedSrc.includes('min-h-[90px]'), 'clip player does not use a tiny under-description banner')
...
assert(exoSrc.includes('FILL_TIMEOUT_MS = 2500'), 'empty in-feed ads give up in 2.5s so the reel does not stall')
assert(exoSrc.includes('fillFrame'), 'display ads can fill clip/pic frames')
assert(exoSrc.includes('pointer-events-auto'), 'display ad slots accept clicks')
assert(exoSrc.includes('touch-pan-y'), 'display slots allow the reel to keep scrolling')
assert(!exoSrc.includes('onPointerDown'), 'display slots do not swallow swipe')
...
assert(exoSrc.includes('fillFrame'), 'exoclick units can fill the clip/pic frame')
assert(exoSrc.includes('slotHasAd'), 'display slots detect the sibling ExoClick injects next to <ins>')
assert(exoSrc.includes('child.tagName === \'INS\''), 'display slots also count creatives that land inside <ins>')
...
assert(shortsFeedSrc.includes('InFeedAd'), 'clip player ads use the shared in-feed component')
assert(shortsFeedSrc.includes('ClipFeedAdSlide'), 'clip scroll ads are full-size slides')
...
assert(picsSrc.includes('mixFeedAds'), 'pic reel mixes full-size scroll ads')
...
assert(authOwner.includes('nobody else can see or play them') || authOwner.includes('device-only session'), 'owner login refuses device-only sessions when cloud is on')
assert(authOwner.includes('setUser(null)'), 'no supabase session clears device-only users')
assert(authOwner.includes('auth.signUp'), 'owner first login can create the hosted account')
...
const officialAt = authCtx.indexOf('findOfficialLogin')
const namedAt = authCtx.indexOf('findNamedAccountLogin')
