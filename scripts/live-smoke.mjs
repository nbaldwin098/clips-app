...
import { findOwnerLogin, OWNER_LOGIN } from '../src/data/ownerLogin.js'
...
assert(brandSrc.includes('Your calabi code is'), 'sms template says calabi')
...
assert(applySrc.includes('Anyone with an account can upload') || applySrc.includes('Anyone signed in can upload'), 'anyone can post without applying')
assert(!applySrc.includes('Submit application'), 'creator application form is gone')
assert(applySrc.includes('CONTENT_RULES_SHORT'), 'create page still shows content rules')
...
assert(ownerHashes.some((h) => ownerHashMatches('Calabi24', h)), 'simple owner password hash is stored')
assert(ownerHashes.some((h) => ownerHashMatches('Calabi-Cs1-24!', h)), 'previous owner password hash is still accepted')
...
assert(sideSrc.includes('Short videos'), 'sidebar no longer says Clips')
const commentsSrc = readFileSync(new URL('../src/components/CommentsPanel.jsx', import.meta.url), 'utf8')
assert(commentsSrc.includes('subscribersLabel'), 'comments show subscriber counts')
assert(commentsSrc.includes('SubscribeButton'), 'comments have a subscribe button')
const orgSrc0 = readFileSync(new URL('../src/lib/orgConfig.js', import.meta.url), 'utf8')
assert(orgSrc0.includes('publicOrigin'), 'reset links use the public site url')
assert(orgSrc0.includes("productName: 'calabi'"), 'product name is calabi')
