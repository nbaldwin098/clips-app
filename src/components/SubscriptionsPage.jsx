  ...
  if (!isAuthenticated) {
    return (
      <div className="p-6 max-w-md mx-auto text-sm text-zinc-400">
        <button type="button" onClick={onOpenAuth} className="text-[#007ACC] font-medium">Sign in</button> to see subscriptions.
      </div>
    )
  }
        ...
        <div className="space-y-2">
          {channels.map((c) => (
            <div key={c.id} className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-[#121218] px-4 py-3">
              <div className="h-10 w-10 rounded-full bg-[#007acc]/20 text-[#007ACC] flex items-center justify-center text-sm font-semibold overflow-hidden">
