/**
 * Simple black admin dashboard chrome. Same tokens as the site.
 */
...
export function DashAreaChart({ seriesA = [], seriesB = [], labels = [], height = 180 }) {
  ...
  const area = (arr, color) => {
    ...
    return (
      <polygon points={`${fx},${h - pad} ${line} ${lx},${h - pad}`} fill={color} opacity="0.18" />
    )
  }
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto" role="img" aria-label="Chart">
      ...
      <polyline fill="none" stroke="#71717a" strokeWidth="2" points={toPts(seriesB)} />
      {labels.length ? (
    ...
    <div className="flex items-end gap-2 h-40">
      {values.map((v, i) => (
        <div key={`${labels[i] || i}`} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
          <div className="w-full bg-white/80" style={{ height: `${(v / max) * 100}%`, minHeight: 4 }} />
...
export default function DashboardShell({
  ...
  return (
    <div className={cn('h-[calc(100dvh-3.5rem)] min-h-[480px] flex bg-[#0f0f0f] text-zinc-100 overflow-hidden', className)} data-dash="admin">
      <aside className="hidden md:flex w-52 shrink-0 flex-col bg-[#0f0f0f] text-zinc-200 border-r border-[#272727]">
                ...
                {g.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'w-full flex items-center gap-2.5 h-9 px-3 text-sm text-left',
