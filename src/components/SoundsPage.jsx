    ...
    <div className="p-4 md:p-6 max-w-[900px] mx-auto w-full">
      ...
      <form onSubmit={create} className="mb-6 rounded-xl border border-zinc-800 bg-[#121218] p-4 space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <button type="submit" className="h-10 px-4 rounded-lg bg-white text-black text-sm">Save sound</button>
        </div>
      </form>
      {mine.length > 0 && (
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {SOUND_CATEGORIES.map((c) => (
          <button key={c} type="button" onClick={() => setCat(c)} className={`h-8 px-3 rounded-full text-xs font-medium ${cat === c ? 'bg-white text-black' : 'bg-zinc-900 text-zinc-400 border border-zinc-800'}`}>{c}</button>
        ))}
      </div>
      <div className="space-y-2">
        {list.map((s) => (
          <div key={s.id} className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-[#121218] px-4 py-3">
            <div className="h-10 w-10 rounded-lg bg-white/15 flex items-center justify-center"><Music className="h-4 w-4 text-white" /></div>
