...
export default function CommentsPanel({ contentId, creatorId }) {
  ...
  const sorted = [...top].sort((a, b) =>
    sort === 'new' ? new Date(b.createdAt) - new Date(a.createdAt) : (b.likes || 0) - (a.likes || 0)
  )
  return (
          ...
          return (
            <div key={c.id} className="text-sm">
              <div className="flex gap-2">
                <div className="h-8 w-8 rounded-full bg-[#007ACC]/20 text-[#007ACC] flex items-center justify-center text-xs font-semibold shrink-0">{(c.handle || '?')[0].toUpperCase()}</div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-zinc-400">@{c.handle}{c.pinned && <span className="ml-2 text-[#007ACC]">Pinned</span>}{c.hearted && <span className="ml-2 text-rose-400">♥</span>}</p>
                  ...
                  {replies.length > 0 && (
                    <div className="mt-2 ml-2 space-y-2 border-l border-zinc-800 pl-3">
                      {replies.map((r) => (
                        <div key={r.id}><p className="text-xs text-zinc-400">@{r.handle}</p><p className="text-zinc-200 text-sm">{r.text}</p></div>
                      ))}
                    </div>
                  )}
                </div>
