...
export const SETTINGS_FIELD =
  'mt-1 w-full h-10 rounded-lg border border-zinc-800 bg-[#000000] px-3 text-sm text-zinc-100'

export const SETTINGS_TEXTAREA =
  'mt-1 w-full rounded-lg border border-zinc-800 bg-[#000000] px-3 py-2 text-sm text-zinc-100'
...
export function SettingsSection({ title, description, divider = false, children, className }) {
  return (
    <section className={cn(divider && 'pt-6 border-t border-zinc-800', 'space-y-4', className)}>
...
export function SettingsCard({ title, description, children, className, headerAction }) {
  return (
    <div className={cn('rounded-xl border border-zinc-800 bg-[#0c0c10] overflow-hidden', className)}>
...
export function SettingsKpiGrid({ items, columns = 4 }) {
  ...
  return (
    <div className={cn('grid gap-3', gridClass)}>
      {items.map((item) => (
        <div key={item.label} className="rounded-xl border border-zinc-800 bg-[#0c0c10] p-4">
          ...
          {item.hint ? <p className="mt-1 text-[11px] text-zinc-600">{item.hint}</p> : null}
        </div>
      ))}
    </div>
  )
      ...
      {tabs.map((tab) => (
        <button
          ...
          className={cn(
            ...
            active === tab.id ? 'text-white border-white' : 'text-zinc-500 border-transparent hover:text-white'
          )}
        >
          {tab.label}
        </button>
      ...
      {ranges.map((range) => (
        <button
          ...
          className={cn(
            ...
            value === range.id ? 'bg-white text-black' : 'text-zinc-400 hover:text-white'
          )}
        >
          {range.label}
        </button>
...
export function SettingsField({ label, hint, children, className }) {
  return (
    <label className={cn('block', className)}>
      {label ? <span className="text-xs font-medium text-zinc-400">{label}</span> : null}
      {children}
  ...
  const toneClass =
    tone === 'warn'
      ...
      : tone === 'success'
        ...
        : 'border-zinc-800 bg-[#121218] text-zinc-300'
  return (
...
export function SettingsTable({ columns, rows, emptyMessage = 'Nothing to list.' }) {
  if (!rows?.length) {
    return <p className="text-sm text-zinc-500 text-center py-12">{emptyMessage}</p>
  }
  return (
              ...
              {columns.map((col) => (
                <td
                  ...
                  className={cn(
                    ...
                    col.muted ? 'text-zinc-500' : 'text-white',
