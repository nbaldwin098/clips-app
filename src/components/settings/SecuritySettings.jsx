...
export default function SecuritySettings() {
  ...
  const clearLocal = () => {
    if (!confirm('Clear all local calabi data on this device? You will be signed out.')) return
    lsRemove('user')
    lsRemove('mode')
    lsRemove('imports')
    lsRemove('liked')
    ...
    <div className="space-y-8 text-zinc-200">
      ...
      <section className="pt-6 border-t border-zinc-800 space-y-3">
        ...
        <p className="text-sm text-zinc-500">
          An authenticator app on your phone. This is real 2FA for a calabi account — not a fake checkbox.
        </p>
        {!synced ? (
          <p className="text-sm text-zinc-400">Sign in with a synced calabi account to turn on 2FA.</p>
        ) : totpOn ? (
