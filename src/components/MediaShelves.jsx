            ...
            {pics.map((pic) => (
              ...
              >
                {(pic.thumbUrl || pic.mediaUrl) ? (
                  <img src={pic.thumbUrl || pic.mediaUrl} alt="" className="absolute inset-0 h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
                ) : null}
                ...
                <span className="absolute bottom-1.5 left-1.5 right-1.5 text-[11px] text-white line-clamp-2 opacity-0 group-hover:opacity-100">
                  {pic.title || viewsLabel(getViews(pic.id))}
                </span>
                {pic.durationSec > 0 ? (
                  <span className="absolute top-1.5 right-1.5 text-[10px] bg-black/80 text-white rounded px-1">{formatDuration(pic.durationSec)}</span>
                ) : null}
              </button>
            ))}
          </div>
