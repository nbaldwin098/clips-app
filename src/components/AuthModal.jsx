...
export default function AuthModal({ open, onClose, initialMode = 'signin' }) {
  ...
  const findUsername = () => {
    ...
    void match
    setInfo('If that email is on calabi, your @username is on your profile after you sign in. Check inbox if you forgot your password.')
  }
      ...
      <div className="w-full max-w-md rounded-2xl border border-[#2f2f37] bg-[#1f1f23] shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="auth-modal-title">
        ...
        <div className="p-5">
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            {mode === 'forgot-pass' && (
              <p className="text-xs text-zinc-500">We’ll email a calabi reset link if this address has an account.</p>
            )}
            {mode === 'forgot-user' && (
