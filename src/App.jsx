...
function AppShell() {
  ...
  useEffect(() => {
    ...
    if (claimed.kind === 'premium') addPremiumSub(user.id, claimed.creatorId || checkoutTarget.id || user.id)
  }, [user?.id, routeParams, checkoutTarget.id])

  useEffect(() => {
    const pull = async () => {
      if (typeof document !== 'undefined' && document.visibilityState === 'hidden') return
      try { await syncContentFromCloud(user) } catch {}
    ...
    const interval = setInterval(pull, user?.id ? 45_000 : 90_000)
    const onFocus = () => pull()
  ...
  const applyRoute = () => {
    ...
    const LEGAL_ALIASES = { terms: 'legal-tos', tos: 'legal-tos', privacy: 'legal-privacy', 'creator-agreement': 'legal-creator', guidelines: 'legal-community' }
    const kind = LEGAL_ALIASES[parsed.kind] || parsed.kind
    const rawRouteId = parsed.id
    const params = parsed.params
    const id = rawRouteId && typeof rawRouteId === 'object' ? String(rawRouteId.id || '') : String(rawRouteId || '')
    ...
    if (kind === 'checkout') {
      ...
      if (creatorId) setCheckoutTarget({ id: creatorId, handle: String(params?.handle || params?.h || '').replace(/^@/, '') })
      setView('checkout')
      setRouteId(id || '')
      return
    }
  ...
  const openWatch = (itemOrId) => {
    ...
    if (!item) return
    stashWatchItem(item)
    if (item.type === 'pic') { openPic(item); return }
    if (item.type === 'short') { setMiniItem(null); navigate('clips', item.id); return }
    setMiniItem(null)
  ...
  const openCreate = (kind) => {
    if (!isAuthenticated) { setAuthOpen(true); return }
    if (kind === 'video') { openUpload('video'); return }
    if (kind === 'clip') { openUpload('short'); return }
    if (kind === 'live') navigate('live')
    if (kind === 'pic') navigate('pics')
  }
  ...
  const selectLiveStreamFromSidebar = (entry) => { setFocusedLiveStream(entry); navigate('live') }
  const toggleChat = () => {
    if (typeof window !== 'undefined' && window.innerWidth < 1024) setMobileChatOpen((v) => !v)
    else setChatCollapsed((prev) => { const next = !prev; lsSet('chat_collapsed', next); return next })
  }

  const renderMain = () => {
    if (!KNOWN_VIEWS.has(view)) return <NotFoundPage onNavigate={navigate} />
    const needsAuth = view === 'dashboard' || view === 'settings' || view === 'channel' || view === 'analytics' || view === 'studio-tools' || view === 'calabi-studio' || view === 'vods' || view === 'verify'
    ...
    switch (view) {
      ...
      case 'watch': return <WatchPage itemId={routeId} startAt={Number(routeParams.t) || 0} onBack={() => navigate('home')} onPlayItem={openWatch} onOpenSound={openSound} onOpenTag={openTag} onOpenProfile={openProfile} onOpenAuth={openAuth} onStitch={openStitch} />
      case 'sound': return <SoundPage soundKey={routeId} onNavigate={navigate} onPlayItem={openWatch} onUseSound={useSound} />
      case 'tag': return <TagPage tag={routeId} onNavigate={navigate} onPlayItem={openWatch} onOpenPic={openPic} onOpenTag={openTag} />
      case 'live': return <LiveView focusedStream={focusedLiveStream} onFocusStream={focusLiveStream} onOpenAuth={openAuth} onNavigate={navigate} />
      case 'dashboard': return <CreatorStudio onOpenImport={openImport} onOpenUpload={openUpload} onNavigate={navigate} onPlayItem={openWatch} onOpenAuth={openAuth} initialSection={routeId || 'overview'} initialSettingsPage={routeParams.tab || 'chat'} />
      case 'vods': return <CreatorStudio onOpenImport={openImport} onOpenUpload={openUpload} onNavigate={navigate} onPlayItem={openWatch} onOpenAuth={openAuth} initialSection="vods" />
      case 'wallet':
      case 'calabi-cash': return <WalletSettings onNavigate={navigate} onOpenAuth={openAuth} initialTab={routeParams.tab} />
      case 'stream-settings': return <CreatorStudio onOpenImport={openImport} onOpenUpload={openUpload} onNavigate={navigate} onPlayItem={openWatch} onOpenAuth={openAuth} initialSection="stream" />
      case 'analytics': return <CreatorStudio onOpenImport={openImport} onOpenUpload={openUpload} onNavigate={navigate} onPlayItem={openWatch} onOpenAuth={openAuth} initialSection="analytics" />
      ...
      case 'verify': return <CreatorStudio onOpenImport={openImport} onOpenUpload={openUpload} onNavigate={navigate} onPlayItem={openWatch} onOpenAuth={openAuth} initialSection="verify" />
      ...
      case 'shop':
      case 'marketplace': return <ShopPage onNavigate={navigate} onOpenAuth={openAuth} />
      case 'seller':
      case 'seller-portal': return <SellerPortal onNavigate={navigate} onOpenAuth={openAuth} />
      ...
      case 'messages': return <MessagesPage onNavigate={navigate} onOpenAuth={openAuth} initialPeerId={routeParams.u || routeId || ''} initialPeerHandle={routeParams.h || ''} />
      case 'appeals': return <AppealsPage onNavigate={navigate} onOpenAuth={openAuth} />
      case 'rewards': return <WalletSettings onNavigate={navigate} onOpenAuth={openAuth} />
  ...
  return (
    <div className="h-dvh overflow-hidden bg-[#000000] text-zinc-100 flex flex-col selection:bg-white selection:text-black">
      ...
      <UploadModal key={`${uploadKind}-${uploadSound?.id || 'none'}-${uploadStitch?.id || 'none'}`} open={uploadOpen} initialKind={uploadKind} initialSound={uploadSound} initialStitch={uploadStitch} onClose={() => { setUploadOpen(false); setUploadSound(null); setUploadStitch(null) }} onOpenAuth={openAuth} />
      ...
      {miniItem && view !== 'watch' && (
        <MiniPlayer item={miniItem} onExpand={() => openWatch(miniItem)} onClose={() => setMiniItem(null)} />
      )}
    </div>
  )
}
