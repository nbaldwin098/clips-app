...
| BUG-021 | iOS | Keep original-file upload path (no WebM re-encode) | #187 — regression test + WebM refuse; default stays original-file | `doing` |
| BUG-022 | Delete | Cloud delete path for admin/creator — verify on Node deploy | #187 — `deleteCatalogItem` + `deleteHostedMedia` (signed/public/path, cloud row fetch) | `doing` |
| BUG-023 | Chat | Live chat cloud sync reliability | #187 — reconnect/poll fallback + shared subscribe hub + channel key normalize | `doing` |
...
| BUG-048 | Security | Review RLS on `videos` + storage policies after bucket recreate | #187 — 0028 WITH CHECK, admin delete, unlisted via `get_video_by_id` | `doing` |
