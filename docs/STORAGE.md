...
| **Play** | Public `getPublicUrl()` on bucket `clips`. Private *catalog* rows are owner-only (`0027`/`0028`); objects with a public URL still load if you have the link. |
| **Delete** | `deleteCatalogItem({ intentional: true })` removes the catalog row, hosted Storage objects, and the IndexedDB blob. |
...
2. **`0028_videos_rls_storage_audit.sql`** — tighter RLS (unlisted not listed, WITH CHECK, admin delete). Already on `main`.
3. **`0029_clips_bucket_limits.sql`** — `file_size_limit` 2 GiB + MIME allowlist on bucket `clips`.

`0003` still creates the public bucket. `0027`–`0029` tighten it. Until they run, production may still have the older “everyone can SELECT videos” policy.
